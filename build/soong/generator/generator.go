...
func init() {
	android.RegisterModuleType("p404_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = p404ExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = p404ExpandVariables(ctx, dep_file)
	...
	cmd := p404ExpandVariables(ctx, String(g.properties.Cmd))

...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/404/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch p404_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/p404-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.p404.device | grep -q "$P404_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $P404_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function p404remote()
{
    ...
    local P404_USER=$(git config --get review.review.p404.org.username)
    if [ -z "$P404_USER" ]
    then
        git remote add p404 ssh://review.p404.org:29418/$PFX$PROJECT
    else
        git remote add p404 ssh://$P404_USER@review.p404.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'p404' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.p404.device | grep -q "$P404_BUILD");
    then
...
function p404gerrit() {
    ...
    local user=`git config --get review.review.p404os.org.username`
...
function p404rebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "404 Gerrit Rebase Usage: "
        echo "      p404rebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.p404os.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.p404.device | grep -q "$P404_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $P404_FIXUP_COMMON_OUT ]; then
